...
func loadFonts() {
	...
	log.Println("[Font] WARNING: No suitable font found. Text rendering may not work.")
}
	...
	if resp.StatusCode == 401 && RT != "" && CS != "" {
		...
		if err != nil {
			...
			if newToken, ok := refRes["access_token"].(string); ok {
				AT = newToken
				log.Println("[API INFO] トークンのリフレッシュに成功しました！")
				req.Header.Set("Authorization", "Bearer "+AT)
...
func renderM32() {
	...
	for i, l := range labels {
		sdeck.FillImage(i, keyTextBg(l, color.RGBA{20, 20, 20, 255}))
	}
	sdeck.FillImage(14, keyTextBg("ホーム", color.RGBA{0, 40, 40, 255}))
	deckMu.Unlock()
}
...
func ircSend(ch, msg string) {
	if ircConn != nil {
		fmt.Fprintf(ircConn, "PRIVMSG #%s :%s\r\n", ch, msg)
	}
}
...
func mainLoop() {
	...
	for {
		...
		if page != TW && now.Sub(lastInput).Seconds() > IDLE_TIMEOUT {
			show(TW, "", false)
		}
