...
### 2026-06-02: config.json優先度を修正
- **問題**: env var > config.json の優先順位だったため、staleなenv varが間違ったclient secretで上書きし続けていた。curlでの直接検証でcredentials自体は有効 (`grant_type=client_credentials` 成功)
- **修正**: 優先順位を config.json > env var > default に変更。config.jsonをアプリの正規保存先とし、env varは手動上書き用に降格
- **検証**: `curl -s -X POST https://id.twitch.tv/oauth2/token -d "client_id=zl3b..." -d "client_secret=i80f..." -d "grant_type=client_credentials"` → 成功 (access_token取得)

### 2026-06-02: saveEnvVarsのenv var永続化を廃止
- **問題**: saveEnvVarsがbashrcに`export TWITCH_CLIENT_SECRET`等を書き込むため、一度間違った値が入るとenv varが最優先される設計上、抜け出せなくなっていた
- **修正**: saveEnvVarsからbashrcへの書き込みを削除。永続化はconfig.json (`~/.config/streamdeck-twitch/config.json`) のみ行う。env varは上級者向けの手動上書きとしてのみ使用

### 2026-06-02: OAページ簡略化
- **問題**: Authボタンがcallbackサーバー経由で自動OAuthフローを実行するようになったが、Get/Save envボタンが残っておりユーザーの手間が増えていた
- **修正**: OAページからGet(クリップボードから取得)とSave env(手動保存)ボタンを削除。Authボタンがauth → get → saveを自動実行する
- **追加**: exchangeCodeForTokensに"invalid client secret"検出時、明確な日本語エラーメッセージを表示
- **追加**: startOAuthにチャネルドレイン処理を追加し、前回のOAuth試行で残ったstaleチャネル値をクリア
- **credentialsリセット**: 誤ったcredentialsがenv var (bashrc) とconfig.jsonに保存されていた問題を修正。config.json削除でデフォルト値（空文字）が使用されるようになった

### credentials: config.jsonが標準の保存先
- `~/.config/streamdeck-twitch/config.json` がメインのcredentials保存先
- env var (`TWITCH_CLIENT_ID`, `TWITCH_CLIENT_SECRET` 等) は手動設定用の上書き手段。saveEnvVarsはenv varを書き込まなくなった
